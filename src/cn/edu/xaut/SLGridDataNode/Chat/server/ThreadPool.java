...
public String getInfo() {
    ...
    for (int i = 0; i < workers.length; i++) {
        sb.append("\nWorker ").append(i).append(" is ").append((workers[i].isWaiting()) ? "Waiting." : "Running.");
    }
    return sb.toString();
}
                ...
                try {
                    ...
                    if (debug) {
                        ...
                        if (r.getFinishTime().getTime()
                            ...
                            taskLogger.debug("longer execution time. " + r.info() + ",<" + index + ">,time:" + (r.getFinishTime().getTime() - r.getBeginExceuteTime().getTime()));
                        }
                    }
                } catch (Exception e) {
                    logger.error(e);
