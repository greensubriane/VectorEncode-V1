            ...
            if ((inmsg = in.readLine()) != null) {
                ...
                if (inmsg.startsWith("old")) {
                    ...
                    if (sendfile[0].compareTo("cancelsendfile") == 0) {
                        ...
                        if (sendfile[2].equals(myName)) { // 如果是发给自己的消息
 ...
 */
public void setSendthread(sendfilethread sendthread) {
...
public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == sendfile) {// 发送文件按钮
        if (withWho.endsWith("所有人")) {
            ...
            if (sendfilename.length() == 0)// 没有获得需要发送的文件
            {
                ...
                setSendthread(new sendfilethread());
                sendboolean = true;
    ...
    if (e.getSource() == sentButton) {// 如果监听到发送信息按钮被点击
        try {
    ...
    if (e.getSource() == perponsComboBox) {// 如果监听到下拉菜单信息
        withWho = (String) perponsComboBox.getSelectedItem(); // 获得选择的名称

    }

    if (e.getSource() == menuItem) {// 如果监听到右击清空主聊天频道被选中
        commonArea.setText("");
    }
    if (e.getSource() == cMenuItem) {// 如果监听到右击清空私聊天频道被选中
        myMsgArea.setText("");
    }
...
public void keyPressed(KeyEvent e) {
    if (e.getKeyCode() == 10) { // 监听到回车键则输出信息
        try {
            mywords = inMsgField.getText();
            if ((mywords.trim()).length() != 0) { // 不能发送空消息也不能都发空格
                if (withWho.equals("所有人")) {
